...
public class Test01_arraylist {
	public static void main(String[] args) {
		...
		int len = ((String) arr[0]).length(); //arr[]은 Object이지만 typecasting해줌

		ArrayList lst = new ArrayList(); //type 안정해줬으니까 Object
		//모든 타입의 객체들을 저장할 수 있다 그러나 비추다! - 이유는?
		String s2 = "java";
		Integer i2 = 123;
		lst.add(s2);
		lst.add(i2);
		...
		//. 다음에 나타나는 메소드를 확인
		//int len = (lst.get(0)).length();//에러! - Object에 length()가 없다
		int len2 = ((String)lst.get(0)).length(); //String으로 하려면 typecasting

		System.out.println("len2 = " + len2);

		ArrayList<String> list = new ArrayList<>();//String 객체로 제한 > 바람직한 형태이다 
		// ArrayList<String> list = new ArrayList<String>(); type 앞이랑 같으면 new 뒤에는 생략가능

		list.add("서울");		list.add("북경");		list.add("상해");
		list.add("서울");		list.add("도쿄");		list.add("뉴욕");
		int len_string = list.get(0).length(); // 오류X ! - list는 string 객체이다 그 차이는(원인은?)
